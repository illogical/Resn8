...
interface ScanDao {
    ...
    suspend fun getScanRunById(id: String): ScanRunEntity?

    @Query("SELECT * FROM scan_runs WHERE sourceId = :sourceId AND status = 'IN_PROGRESS' ORDER BY startedAt DESC LIMIT 1")
    suspend fun getActiveScanRun(sourceId: String): ScanRunEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertScanRun(scanRun: ScanRunEntity)

    @Query("UPDATE scan_runs SET status = :status, completedAt = :completedAt, errorSummary = :errorSummary WHERE id = :scanId")
    suspend fun updateScanRunStatus(scanId: String, status: String, completedAt: Long?, errorSummary: String? = null)
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertStagedFolders(folders: List<StagedFolderEntity>)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertStagedMedia(media: List<StagedMediaEntity>)

    @Query("SELECT * FROM staged_folders WHERE scanId = :scanId")
    suspend fun getStagedFolders(scanId: String): List<StagedFolderEntity>

    @Query("SELECT * FROM staged_media WHERE scanId = :scanId")
    suspend fun getStagedMedia(scanId: String): List<StagedMediaEntity>

    @Query("SELECT COUNT(*) FROM staged_media WHERE scanId = :scanId")
    suspend fun countStagedMedia(scanId: String): Int

    @Query("SELECT * FROM staged_media WHERE scanId = :scanId ORDER BY id LIMIT :limit OFFSET :offset")
    suspend fun getStagedMediaBatch(scanId: String, limit: Int, offset: Int): List<StagedMediaEntity>
    ...
    suspend fun getStagedFolderBatch(scanId: String, limit: Int, offset: Int): List<StagedFolderEntity>

    @Query("UPDATE staged_media SET resolvedMediaId = :mediaId, resolvedFolderId = :folderId WHERE id = :stagedId")
    suspend fun setResolvedMedia(stagedId: String, mediaId: String, folderId: String)

    @Query("UPDATE staged_folders SET resolvedFolderId = :folderId WHERE id = :stagedId")
    suspend fun setResolvedFolder(stagedId: String, folderId: String)

    @Query("DELETE FROM staged_folders WHERE scanId = :scanId")
    suspend fun deleteStagedFolders(scanId: String)

    @Query("DELETE FROM staged_media WHERE scanId = :scanId")
    suspend fun deleteStagedMedia(scanId: String)
}
