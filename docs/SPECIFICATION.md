...
- `likeScore` is an integer with default `0` and minimum `-1`; positive values have no product-defined maximum.
- Every explicit Like action atomically adds `1`. Dislike atomically subtracts `1` only while the score is above `-1`; repeated Dislike actions at `-1` are idempotent.
- A score greater than zero is liked, zero is neutral, and less than zero is disliked.
- The UI displays `+N` for positive scores and hides the numeric count at `0` and `-1`. Both actions remain visible; pressing the opposite action adjusts the score toward neutral.
...
The now-playing surface is non-scrollable and always exposes title, profile-appropriate artist/album when available, elapsed/duration position, seek control, Previous, Play/Pause, Next, Like, Dislike, the positive score when present, and Add to Playlist. Add to Playlist is right-aligned above the artwork/content region; the centered rating cluster keeps stable geometry while hiding score text at `0` and `-1`. Controls claim layout space before artwork and retain at least 48dp targets. Artwork resolves external or embedded indexed-file art asynchronously through a bounded cache, uses a stable placeholder while unavailable/loading, shrinks within the remaining portrait or landscape region, and may be omitted when the available square would be smaller than 72dp. Audio Files use a compact two-line title and never show artist or album. Each explicit queue persists its Playlist, Album, Artist, Folder, or All Tracks browse origin; the matching single-line icon-and-label app-bar action opens that exact context without replacing the queue. Now Playing does not expose a separate View Queue action, though saved-queue playback and the compatibility Queue route remain intact.

Add to Playlist remains available through a visible button. Double-tapping the left or right half of the artwork region optionally seeks backward or forward ten seconds with brief directional motion feedback. The visible seek slider remains the discoverable and accessibility-compatible path.
